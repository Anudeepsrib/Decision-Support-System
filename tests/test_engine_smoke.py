...
Run with: python tests/test_engine_smoke.py
"""
from backend.engine.constants import KSERC, T_AND_D_LOSS_TRAJECTORY
from backend.engine.rule_engine import RuleEngine, CostInput

engine = RuleEngine(KSERC)
passed = 0

# Test 1: Controllable Gain
inp1 = CostInput(head="O&M", category="Controllable", sbu_code="SBU-D",
...
# Test 3: Controllable Loss
inp3 = CostInput(head="O&M", category="Controllable", sbu_code="SBU-D",
...
try:
    bad = CostInput(head="O&M", category="Controllable", sbu_code="SBU-D",
...
assert KSERC.NORMATIVE_INTEREST_RATE == expected_rate, f"Expected {expected_rate}, got {KSERC.NORMATIVE_INTEREST_RATE}"
print(f"[PASS] TEST 6 — Normative Interest Rate: {KSERC.NORMATIVE_INTEREST_RATE} (derived, not hardcoded)")
passed += 1

# Test 7: Uncontrollable pass-through
inp7 = CostInput(head="Power_Purchase", category="Uncontrollable", sbu_code="SBU-G",
...
print(f"\n{'='*50}")
print(f"ALL {passed}/7 SMOKE TESTS PASSED ✅")
print(f"{'='*50}")
